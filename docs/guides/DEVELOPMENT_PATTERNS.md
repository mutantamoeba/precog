...
**Status:** ✅ Current
**Changes in V1.43:**
- **Added Pattern 89: Post-Merge Sentinel Recovery (ALWAYS When CI Wall-Clock Races Reviewer Dispatch on Auto-Merge-Armed PRs)** AND **Pattern 90: Auto-Merge State Hygiene (ALWAYS After Any Push to an Auto-Merge-Armed PR)** — promoted as **two separate patterns** (Option X), not the consolidated single-pattern shape Pattern 88 (V1.41) precedented. **Why two patterns rather than one:** the two rules address truly orthogonal failure modes despite both touching "post-merge / auto-merge state surfaces drift." Pattern 89 is a *quality-discipline rule* about cold-context coverage (when reviewer dispatch wall-clock loses to CI green wall-clock, dispatch the reviewer post-merge — the audit is not a wasted retry, it provides genuine SSOT-against-merged-state coverage that pre-merge frames under timing pressure can miss). Pattern 90 is an *operational `gh` CLI mechanics rule* about armed-state fragility (auto-merge can silently fail in two distinct shapes — `mergeStateStatus: BEHIND` blocks an armed merge, and any push to a PR head silently disarms auto-merge — both with different `gh` diagnostic paths). The reader hitting "my Sentinel raced auto-merge and missed a real finding" does not need to learn about `mergeStateStatus: BEHIND`, and vice versa. Pattern 88's paired-rule precedent applied because both 88a + 88b shared the same surface (PG partitioned tables) AND the same incident (slot 0078 PR #1120); Pattern 89 + 90 share neither surface (dispatch discipline vs `gh` CLI mechanics) nor a single origin incident (Pattern 89 origin = session 88 PR #1131 SDL Framework V1.0; Pattern 90 origin = session 72 PR #1008 BEHIND-state diagnosis + session 86 PR #1120 disarm — three separate sessions across 14 cycles). Cross-reference searchability also improves: a future session memo citing "we hit Pattern 90b" is more discoverable than "Pattern 89.something" sub-rule indexing.
...
| 88 | #1131 (SDL Framework V1.0) | (no incident — instance for cross-reference to Pattern 89) | Auto-merge fired correctly; the race against Hermione's dispatch was the orthogonal Pattern 89 surface, not a Pattern 90 failure | n/a — Pattern 89 fired here, not Pattern 90 |
