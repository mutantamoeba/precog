...
- **Testing:** `TESTING_STRATEGY_V3.5.md` - Comprehensive testing infrastructure
- **Requirements:** `MASTER_REQUIREMENTS_V2.22.md` - REQ-VALIDATION-001, REQ-VALIDATION-002
- **ADRs:** `ARCHITECTURE_DECISIONS_V2.29.md` - ADR-048 (Ruff), ADR-050 (Doc Validation), ADR-051 (Layered Validation), ADR-054 (Bandit→Ruff Migration), ADR-075 (Multi-Source Warning Governance), ADR-076 (Dynamic Ensemble Weights), ADR-077 (Strategy vs Method Separation)
