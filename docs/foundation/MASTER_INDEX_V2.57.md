...
| **MASTER_INDEX_V2.57.md** | ✅ | v2.57 | `/docs/foundation/` | 0 | All phases | 🔴 Critical | THIS FILE - complete document inventory - **UPDATED V2.57** (Cohort 1 carry-forward amendment session 75 / Issue #1011: ARCHITECTURE_DECISIONS V2.39 → V2.40 with 5 PM-adjudicated decisions — partial unique index for `canonical_participant_roles` cross-domain singleton, `sequence_number` NO DEFAULT ratified, `lifecycle_phase` CHECK + 0077 mirror, Pattern 82 V2 Forward-Only Direction Policy + load-bearing regression test, seed-subquery NULL guard → Pattern 83; ADR_INDEX V1.29 → V1.30 amended ADR-118 row; DEVELOPMENT_PATTERNS V1.36 → V1.37 Pattern 82 V2 + Pattern 83 promoted in parallel PR); V2.56 (Cohort 2 amendment session 73) |
| **ARCHITECTURE_DECISIONS_V2.40.md** | ✅ | v2.40 | `/docs/foundation/` | 0 | Phases 1-10 | 🟡 High | Design rationale with ADR numbers - **UPDATED V2.40** (ADR-118 Cohort 1 carry-forward amendment session 75 / Issue #1011: 5 PM-adjudicated decisions from 3-agent council — partial unique index `canonical_participant_roles (role) WHERE domain_id IS NULL`; `sequence_number` NO DEFAULT ratified; `canonical_events.lifecycle_phase` CHECK NOW + mirror on Migration 0077 `canonical_event_phase_log.phase`; Pattern 82 V2 Forward-Only Direction Policy + load-bearing regression test; seed-subquery NULL guard → Pattern 83 (V1.37); Future-Cohort `canonical_events.(game_id, series_id)` Pattern 82 finding flagged for Migration 0085); supersedes V2.39 (Cohort 2 amendment session 73) |
...
| **ADR_INDEX_V1.30.md** | ✅ | v1.30 | `/docs/foundation/` | 0 | All phases | 🔴 Critical | Systematic catalog of architecture decisions - **UPDATED V1.30** (ADR-118 row amended for Cohort 1 carry-forward v2.40 amendment — Issue #1011 / session 75; supersedes V1.29 Cohort 2 v2.39 amendment session 73) |
