...
| **MASTER_INDEX_V2.56.md** | ✅ | v2.56 | `/docs/foundation/` | 0 | All phases | 🔴 Critical | THIS FILE - complete document inventory - **UPDATED V2.56** (Cohort 2 amendment session 73: ARCHITECTURE_DECISIONS V2.38 → V2.39 with `canonical_markets` DDL tightened, `lifecycle_phase` exclusion + `updated_at` trigger template + Pattern 82 non-application + ON DELETE RESTRICT ratified; ADR_INDEX V1.28 → V1.29 with amended ADR-118 row); V2.55 (Cohort 1 amendment session 72 — 4 lookup tables + `ref_team_id` + `sequence_number`; closes #996) |
| **ARCHITECTURE_DECISIONS_V2.39.md** | ✅ | v2.39 | `/docs/foundation/` | 0 | Phases 1-10 | 🟡 High | Design rationale with ADR numbers - **UPDATED V2.39** (ADR-118 Cohort 2 amendment session 73: `canonical_markets` DDL tightened — audit-column expansion + `market_type_general` CHECK + `updated_at` + BEFORE UPDATE trigger as #1007 retrofit template; explicit non-application prose for `lifecycle_phase` + Pattern 82; `ON DELETE RESTRICT` on `canonical_event_id` ratified; bonus footnotes for Migrations 0076/0078/0083 — Issues #1015/#1016/#1017); supersedes V2.38 (Cohort 1 amendment #996 — 4 lookup tables + `ref_team_id` + `sequence_number`) |
...
| **ADR_INDEX_V1.29.md** | ✅ | v1.29 | `/docs/foundation/` | 0 | All phases | 🔴 Critical | Systematic catalog of architecture decisions - **UPDATED V1.29** (ADR-118 row amended for Cohort 2 v2.39 amendment; supersedes V1.28 — Cohort 1 amendment #996 — 7 tables / 2 migrations with 4 lookup tables + typed back-refs + sequence_number) |
