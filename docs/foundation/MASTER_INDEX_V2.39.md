...
| **TESTING_STRATEGY_V3.2.md** | ✅ | v3.2 | `/docs/foundation/` | 2 | Phases 1-10 | 🔴 Critical | **UPDATED V3.2** - All 8 test types now MANDATORY for ALL tiers (removed phase-based deferral); audit script (scripts/audit_test_type_coverage.py) for enforcement; pre-push hook (Step 11) + CI job for validation; V3.1: Enhanced coverage tier classification based on TDD/security/accuracy emphasis; Position Manager 85%→90% (handles money+real-time); V3.0: 8 test type framework (unit, property, integration, E2E, stress, race, performance, chaos) |
...
| **DEVELOPMENT_PATTERNS_V1.13.md** | ✅ | v1.13 | `/docs/guides/` | All | All phases | 🔴 Critical | **UPDATED V1.13** - Added Pattern 25 (ANSI Escape Code Handling for Cross-Platform CLI Testing) - 25 total patterns; Pattern 25 documents Rich library ANSI escape code handling for Windows CI compatibility: strip_ansi() helper function, regex pattern `\x1b\[[0-9;]*m`, 20+ test assertions fixed in PR #159; cross-references Pattern 5 (Cross-Platform Compatibility), PR #159 |
