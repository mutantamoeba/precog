...
| **TESTING_STRATEGY_V3.5.md** | ✅ | v3.5 | `/docs/foundation/` | 2 | Phases 1-10 | 🔴 Critical | **UPDATED V3.5** - Added Best Practice #6: Test Failure Response Pattern (fix CODE DEFECT, not test expectation); decision tree for test failure response; V3.4: CI-Safe Stress Testing; V3.3: Test Isolation Patterns; V3.2: All 8 test types MANDATORY; V3.0: 8 test type framework |
...
| **DEVELOPMENT_PATTERNS_V1.19.md** | ✅ | v1.19 | `/docs/guides/` | All | All phases | 🔴 Critical | **UPDATED V1.19** - Added Pattern 31 (Pre-Push Log Persistence - JSON summaries + analyze_test_history.py for trend analysis and flaky test detection); 31 total patterns; Issue #174 implementation; cross-references Pattern 29/30 |
...
| **SCHEMA_MIGRATION_WORKFLOW_V2.0.md** | ✅ | v2.0 | `/docs/utility/` | 1.5 | All phases | 🔴 Critical | Comprehensive Alembic-based schema migration workflow: 13 main sections covering complete migration workflow (Plan → Create Alembic Migration → Apply and Verify → Update CRUD → Update Tests → Update Docs → Commit/Deploy); pre-migration checklist, Alembic command reference, rollback procedures, troubleshooting guide; **V2.0 BREAKING CHANGE: Alembic is now the EXCLUSIVE migration tool - removed all references to legacy SQL/Python dual-migration system** |
