...
| **DEVELOPMENT_PATTERNS_V1.14.md** | ✅ | v1.14 | `/docs/guides/` | All | All phases | 🔴 Critical | **UPDATED V1.14** - Added Pattern 26 (Resource Cleanup for Testability) and Pattern 27 (Dependency Injection for Testability) - 27 total patterns; Pattern 26 documents close() methods for HTTP sessions; Pattern 27 documents DI approach for injecting mock dependencies; both patterns support Phase 1.9 test infrastructure; cross-references ADR-079, TESTING_STRATEGY_V3.3 |
...
| **SCHEMA_MIGRATION_WORKFLOW_V1.1.md** | ✅ | v1.1 | `/docs/utility/` | 1.5 | All phases | 🔴 Critical | Comprehensive database schema migration workflow: 13 main sections covering complete migration workflow (Plan → Create Migration → Update CRUD → Update Tests → Run Tests → Update Docs → Commit/Deploy); pre-migration checklist, dual-key pattern code examples, test templates (Pattern 13 compliant - NO mocks), rollback procedures, common scenarios, troubleshooting guide; companion to Pattern 14 (Schema Migration → CRUD Workflow); cross-references ADR-089 (Dual-Key Schema Pattern); **V1.1: Added "SQL vs Python Migrations" section documenting migration type strategy, CI execution order, conflict prevention, and game_states case study** |
