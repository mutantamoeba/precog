...
**Status:** ✅ Current
**Supersedes:** V2.43 (deleted per supersede-and-delete convention; PR #979 / PR #1009 / PR #1018 / PR #1063 / PR #1118 precedent)
**Session:** 89
**Changes in v2.44:**
- **ADR-118 COHORT 5+ MATCHER-ATOMICITY CONTRACT (Issue #1135, session 89):** Codifies the session 89 #1135 council synthesis (Galadriel + Holden 2-agent council + PM synthesis) on `canonical_event_id` binding consistency across `game_states.canonical_event_id` (slot 0080) + `temporal_alignment.canonical_event_id` (slot 0082). Amendment is narrative + acceptance-criteria codification only; no new tables ship under v2.44. Implementation lands in **slot 0083** (Cohort 4 close) for the `temporal_alignment` INSERT trigger + dual-column view, AND in the **Cohort 5+ matcher slot** for the `games` write path + `game_states` INSERT trigger. Closes Issue #1135.
  - **Item 1 — Council ratifies pre-existing v2.43 sub-decision #4** ("Dim + fact canonical linkage is BOTH" — line 17326 + detailed shape at lines 17849-17859). The #1135 council surfaced as principled divergence (Galadriel: Option A denormalized cache via trigger + reconciler health-check; Holden: Option C reconciliation + dim/fact split). PM synthesis (`memory/design_review_1135_synthesis.md`) collapsed the divergence by referencing the existing v2.43 commitment that both council members had under-quoted. The V2.44 amendment is therefore **RATIFICATION**, not new architecture — the architectural commitment was made in V2.43; V2.44 elevates the implementation contract to ADR-level so future Cohort 5+ matcher implementers see it as a binding constraint rather than a memo to discover.
  ...
  - **Item 3 — Writer-side trigger discipline (split across two slots):**
    ...
    - Trigger pattern matches the existing `set_updated_at()` precedent from slot 0076 (Pattern 87 V1.40 ratification): BEFORE INSERT, FOR EACH ROW, plpgsql, respects caller-supplied non-NULL values (allows the future Cohort 5+ matcher to bypass the trigger by passing the value directly during a coordinated bind)
  ...
  - **Item 5 — Reconciler is the sole enforcement surface.** **NO** CHECK constraint or cross-table trigger asserting column agreement:
    ...
    - The `reconcile_canonical_linkage` reconciler (per V2.43 line 17857) gains a query shape comparing `(games.canonical_event_id, game_states.canonical_event_id_for_current_row, temporal_alignment.canonical_event_id_per_game_state)` and surfaces divergences as **warning-severity** entries in `canonical_reconciliation_results` per V2.43 Item 5 baseline (warning-only on Cohort 4+ ship; tightening gated to Cohort 6+ post-baseline)
...
**END OF ARCHITECTURE DECISIONS V2.43**
