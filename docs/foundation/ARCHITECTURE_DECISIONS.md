...
4. **Pattern 82 V2 Forward-Only Direction Policy + mandatory load-bearing regression test (#1011 item 4).**
   ...
   > **Pattern 82 V2 wording (canonical):** see `DEVELOPMENT_PATTERNS.md § Pattern 82 V2 Forward-Only Direction Policy` (line 12239) for the rule, the rationale (preserves polymorphic-overloading future-extensibility per ADR-118 V2.38 decision #5), and the mandatory compensating mechanism (every Pattern 82 application MUST be paired with a load-bearing regression test). ADR-118 V2.40 ratifies this rule for the polymorphic-typed-back-ref family in this codebase; the canonical wording's home is `DEVELOPMENT_PATTERNS.md` per Pattern 73 SSOT (rules → `DEVELOPMENT_PATTERNS.md`, decisions → ADRs). Closes Pattern 73 violation flagged by claude-review on PR #1034 (#1036).
   >
