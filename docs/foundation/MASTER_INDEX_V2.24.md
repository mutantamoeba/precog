...
| **ARCHITECTURE_DECISIONS_V2.17.md** | ✅ | v2.16 | `/docs/foundation/` | 0 | Phases 1-10 | 🟡 High | Design rationale with ADR numbers (87 total) - **UPDATED V2.16** (ADR-086: Schema Classification Field Naming - approach/domain standardization; ADR-087: No Edge Manager Component - calculated outputs pattern) |
...
| **ADR_INDEX_V1.12.md** | ✅ | v1.11 | `/docs/foundation/` | 0 | All phases | 🔴 Critical | Systematic catalog of all 67 architecture decisions - **UPDATED V1.11** (ADR-086: Schema Classification Field Naming - approach/domain; ADR-087: No Edge Manager Component - Phase 1.5) |
...
| **TESTING_STRATEGY_V3.0.md** | ✅ | v3.0 | `/docs/foundation/` | 0.6c | Phases 1-10 | 🟡 High | **UPDATED V3.0** - Comprehensive 8 test type framework addressing Phase 1.5 TDD failure (expanded from 4 types to: unit, property, integration, E2E, stress, race, performance, chaos); mock usage restrictions (REQ-TEST-013: APPROPRIATE for external APIs/time, FORBIDDEN for database/internal logic/config); integration testing philosophy "test with REAL infrastructure not mocks"; test type requirements matrix (module tier × test type); phase-based roadmap (when each type required Phases 1-5); property-based testing with Hypothesis (REQ-TEST-018) |
...
| Document | Status | Version | Location | Phase | Phase Ties | Priority | Notes |
|----------|--------|---------|----------|-------|------------|----------|-------|
| **TESTING_STRATEGY_V3.0.md** | ✅ | v3.0 | `/docs/foundation/` | 0.6c | Phases 1-10 | 🟡 High | **UPDATED V3.0** - Comprehensive 8 test type framework (unit, property, integration, E2E, stress, race, performance, chaos); mock usage restrictions (REQ-TEST-013); "test with REAL infrastructure not mocks" philosophy; test type requirements matrix; phase-based roadmap; Hypothesis property testing (REQ-TEST-018) |
...
| **DEVELOPMENT_PATTERNS_V1.3.md** | ✅ | v1.3 | `/docs/guides/` | All | All phases | 🔴 Critical | **UPDATED V1.3** - Added Pattern 12 (Test Fixture Security Compliance - project-relative fixtures for security-validated functions) + Pattern 13 (Test Coverage Quality - Mock Sparingly, Integrate Thoroughly); Pattern 13 documents critical TDD failure lessons: when to use mocks vs real infrastructure, ALWAYS use conftest.py fixtures (clean_test_data, db_pool, db_cursor), 8 required test types, module coverage standards (Critical Path ≥90%, Manager ≥85%, Infrastructure ≥80%), test review checklist; prevents false confidence from mock-based tests (Strategy Manager: 17/17 passing with mocks → 13/17 failing with real database) |
