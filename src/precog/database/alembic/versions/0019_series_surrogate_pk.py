...
def upgrade() -> None:
    ...
    Steps:
    1. Drop events FK to series (must precede PK drop — dependent objects)
    2. Add SERIAL id column to series
    3. Drop existing PRIMARY KEY on series_id
    4. Set id as new PRIMARY KEY
    5. Add UNIQUE constraint on series_id (business key)
    6. Add UNIQUE(platform_id, external_id) composite constraint
    7. Add series_internal_id INTEGER column to events
    8. Backfill series_internal_id from old VARCHAR series_id
    9. Add FK constraint on series_internal_id
    10. Drop old events.series_id VARCHAR column + index
    11. Add index on events.series_internal_id
    ...
    """
    # ── Step 1: Drop events FK to series FIRST ──
    # Must drop the dependent FK before we can drop the series PK.
    ...
    # ── Step 2: Add SERIAL id column to series ──
    ...
    # ── Step 3: Drop existing PRIMARY KEY on series_id ──
    # Safe now that the events FK has been dropped in Step 1.
    op.execute("""
        ALTER TABLE series
    ...
    # ── Step 5: Add UNIQUE constraint on series_id (business key) ──
    ...
    # ── Step 7: Add series_internal_id INTEGER column to events ──
    ...
    # ── Step 8: Backfill series_internal_id from old series_id VARCHAR ──
    # For existing data: look up the new integer PK from the series table
    # using the old VARCHAR series_id that events still has (not yet dropped).
    op.execute("""
        UPDATE events e
        SET series_internal_id = s.id
        FROM series s
    ...
    # ── Step 10: Drop old events.series_id VARCHAR column + index ──
    ...
    # ── Step 11: Add index on events.series_internal_id ──
