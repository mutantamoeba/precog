"""
Unit Tests for Game Odds Loader.

Tests the data transformation and loading functions for game
odds data (renamed from historical_odds in migration 0048).
...
Usage:
    pytest tests/unit/database/seeding/test_game_odds_loader.py -v
"""
...
class TestSourceNameMapping:
    ...
    def test_known_sources_mapped_correctly(self) -> None:
        ...
        assert SOURCE_NAME_MAPPING["espn_poller"] == "espn_poller"
    ...
    def test_normalize_known_source(self) -> None:
        ...
        assert normalize_source_name("espn_poller") == "espn_poller"
...
class TestBackwardCompatibility:
    """Verify old import paths still work after rename."""

    def test_historical_odds_loader_still_importable(self) -> None:
        """Old import path should still work via shim module."""
        from precog.database.seeding.historical_odds_loader import (
            SOURCE_NAME_MAPPING as OLD_MAPPING,
        )
        from precog.database.seeding.historical_odds_loader import (
            LoadResult as OldLoadResult,
        )
        from precog.database.seeding.historical_odds_loader import (
            normalize_source_name as old_normalize,
        )

        assert OldLoadResult is BatchInsertResult
        assert "espn_poller" in OLD_MAPPING
        assert old_normalize("betting_csv") == "betting_csv"

    def test_backward_compat_function_aliases(self) -> None:
        """Old function names should still be importable."""
        from precog.database.seeding.historical_odds_loader import (
        ...
        # Just verify they're callable (don't actually call DB functions)
        assert callable(bulk_insert_historical_odds)
        assert callable(get_historical_odds_stats)
        assert callable(insert_historical_odds)
