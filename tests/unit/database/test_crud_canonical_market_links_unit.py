"""Unit tests for crud_canonical_market_links module — Pattern 14 step 4 of the
Cohort 3 slot-0072 bundle.

Covers (function-by-function):
    - get_active_link_for_platform_market: happy path (row found), None
      (no active link).
    - get_link_by_id: happy path, None (not found).
    - list_links_for_canonical_market: multiple rows, empty list, ORDER BY
      shape.
    - retire_link: True (row updated) + retire_reason populated, False (no
      row matched), retire_reason=None passthrough, does NOT write
      updated_at directly.

Pattern 43 (mock fidelity) discipline: mocks return the EXACT shape that
the real query returns — full row dicts with all 12 canonical_market_links
...
Reference:
    - ``src/precog/database/crud_canonical_market_links.py``
    ...
    - ADR-118 v2.41 amendment Cohort 3 + S82 build spec § 8 step 4a
"""
...
from precog.database.crud_canonical_market_links import (
    get_active_link_for_platform_market,
    get_link_by_id,
    list_links_for_canonical_market,
    retire_link,
)
...
def _full_row_dict(
    *,
    ...
    """Build a full canonical_market_links row dict matching the real query shape.
    ...
    canonical_market_links row dict look like in tests".
    """
...
@pytest.mark.unit
class TestGetActiveLinkForPlatformMarket:
    """Unit tests for get_active_link_for_platform_market — partial-active SELECT."""

    @patch("precog.database.crud_canonical_market_links.fetch_one")
    def test_returns_row_dict_when_active_link_exists(self, mock_fetch_one):
        ...
        expected_row = _full_row_dict(platform_market_id=100, link_state="active")
        mock_fetch_one.return_value = expected_row

        result = get_active_link_for_platform_market(100)

        assert result == expected_row
        mock_fetch_one.assert_called_once()
        sql, params = mock_fetch_one.call_args[0]
        assert "FROM canonical_market_links" in sql
    ...
    def test_returns_none_when_no_active_link(self, mock_fetch_one):
        """Returns None when no active link exists for the platform market.

        This is the matcher's "create new canonical binding" signal — None
        means no active link, so a fresh active link can be inserted (the
        EXCLUDE partial-active constraint is satisfied by the absence).
        """
        mock_fetch_one.return_value = None

        result = get_active_link_for_platform_market(99999)
    ...
    @patch("precog.database.crud_canonical_market_links.fetch_one")
    def test_query_selects_all_canonical_market_links_columns(self, mock_fetch_one):
        ...
        SELECT would silently pass because the mock dict (built by
        _full_row_dict()) has all keys regardless of what SELECT projects.
        """
        mock_fetch_one.return_value = None

        get_active_link_for_platform_market(100)
...
@pytest.mark.unit
class TestListLinksForCanonicalMarket:
    """Unit tests for list_links_for_canonical_market — fetch_all by FK."""

    @patch("precog.database.crud_canonical_market_links.fetch_all")
    def test_returns_list_when_links_exist(self, mock_fetch_all):
        """Returns a list of full row dicts when links exist for the canonical market."""
        ...
        result = list_links_for_canonical_market(42)
    ...
    def test_returns_empty_list_when_no_links(self, mock_fetch_all):
        """Returns an empty list when no links exist for the canonical market."""
        mock_fetch_all.return_value = []

        result = list_links_for_canonical_market(99999)

        assert result == []
        assert mock_fetch_all.call_args[0][1] == (99999,)

    @patch("precog.database.crud_canonical_market_links.fetch_all")
    def test_query_selects_all_canonical_market_links_columns(self, mock_fetch_all):
        """Pattern 43 fidelity: SELECT projection must include all 12 columns."""
        mock_fetch_all.return_value = []

        list_links_for_canonical_market(42)
...
class TestRetireLink:
    ...
    @patch("precog.database.crud_canonical_market_links.get_cursor")
    def test_returns_true_when_row_updated(self, mock_get_cursor):
        """Returns True when a row was matched and updated (rowcount=1).

        Pattern 73 SSOT discipline assertion: the SQL writes link_state =
        'retired' literally (the canonical retired-state value); the
        retire_reason from the caller is bound as a parameter.
        """
        ...
        assert "UPDATE canonical_market_links" in sql
        assert "link_state = 'retired'" in sql
        assert "retired_at = now()" in sql
        assert "retire_reason = %s" in sql
        assert "WHERE id = %s" in sql
        # Params: (retire_reason, link_id) per the SQL bind order
    ...
    def test_does_not_write_updated_at_directly(self, mock_get_cursor):
        ...
        The BEFORE UPDATE trigger ``trg_canonical_market_links_updated_at``
        refreshes ``updated_at`` automatically.  Callers must rely on the
        trigger; writing updated_at here would be a Pattern 73 violation
        (duplicating trigger behavior in application code).
        """
        mock_cursor = MagicMock()
        mock_cursor.rowcount = 1
        mock_get_cursor.return_value.__enter__ = MagicMock(return_value=mock_cursor)
