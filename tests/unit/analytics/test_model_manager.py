...
def test_create_model_success(manager, model_factory):
    ...
    assert model["model_type"] == "elo"
    assert model["sport"] == "nfl"
...
def test_create_model_minimal_fields(manager):
    ...
    assert model["model_version"] == "1.0"
    assert model["sport"] is None  # Optional field defaults to None
    assert model["description"] is None
    assert model["status"] == "draft"  # Default status


def test_create_model_decimal_conversion(manager, model_factory):
...
def test_create_model_ensemble_type(manager):
...
def test_get_model_by_id(manager, model_factory):
...
def test_get_model_by_name_and_version(manager, model_factory):
...
def test_get_model_requires_identifier(manager):
...
def test_get_model_requires_both_name_and_version(manager):
...
def test_list_models_all(manager, model_factory):
...
def test_list_models_filter_by_status(manager, model_factory):
...
def test_list_models_filter_by_sport(manager, model_factory):
...
def test_list_models_multiple_filters(manager, model_factory):
    ...
    filtered = manager.list_models(sport="nfl", status="draft", model_type="elo")
...
def test_update_metrics_calibration_score(manager, model_factory):
    ...
    updated = manager.update_metrics(
        model_id=model_id,
        calibration_score=Decimal("0.9250"),  # 92.5% calibration
    )

    assert updated["calibration_score"] == Decimal("0.9250")
    assert updated["updated_at"] > model["updated_at"]  # Timestamp updated


def test_update_metrics_accuracy(manager, model_factory):
    """Test updating accuracy metric."""
    model = manager.create_model(**model_factory)
    model_id = model["model_id"]

    # Update accuracy
    updated = manager.update_metrics(model_id=model_id, accuracy=Decimal("0.6800"))  # 68% accuracy

    assert updated["accuracy"] == Decimal("0.6800")


def test_update_metrics_predictions_count(manager, model_factory):
    """Test updating predictions_count metric."""
    model = manager.create_model(**model_factory)
    model_id = model["model_id"]

    # Update predictions_count
    updated = manager.update_metrics(model_id=model_id, predictions_count=150)

    assert updated["predictions_count"] == 150


def test_update_metrics_multiple_fields(manager, model_factory):
    ...
    updated = manager.update_metrics(
        model_id=model_id,
        calibration_score=Decimal("0.9100"),
        accuracy=Decimal("0.6500"),
        predictions_count=200,
    )

    assert updated["calibration_score"] == Decimal("0.9100")
    assert updated["accuracy"] == Decimal("0.6500")
    assert updated["predictions_count"] == 200


def test_update_metrics_no_fields_provided(manager, model_factory):
...
def test_update_metrics_model_not_found(manager):
    """Test update_metrics raises ValueError when model doesn't exist."""
    with pytest.raises(ValueError, match="Model not found"):
        manager.update_metrics(model_id=99999, calibration_score=Decimal("0.90"))
...
def test_update_status_draft_to_testing(manager, model_factory):
...
def test_update_status_testing_to_active(manager, model_factory):
...
def test_update_status_active_to_deprecated(manager, model_factory):
...
def test_update_status_testing_to_draft_revert(manager, model_factory):
...
def test_update_status_invalid_transition_draft_to_active(manager, model_factory):
...
def test_update_status_invalid_transition_deprecated_to_active(manager, model_factory):
...
def test_update_status_model_not_found(manager):
...
def test_config_immutability_no_update_method(manager):
...
def test_config_immutability_requires_new_version(manager, model_factory):
...
def test_decimal_precision_preserved_in_database(manager, model_factory):
...
def test_decimal_precision_no_float_contamination(manager, model_factory):
...
def test_error_handling_invalid_model_type(manager, model_factory):
    """Test creating model with invalid model_type."""
    invalid_config = model_factory.copy()
    invalid_config["model_type"] = "invalid_type"  # Not in [elo, ensemble, ml, hybrid]
...
def test_error_handling_duplicate_name_version(manager, model_factory):
...
def test_status_lifecycle_valid_paths(manager, model_factory):
...
def test_status_lifecycle_terminal_state(manager, model_factory):
...
def test_model_manager_independent_from_strategy_manager(manager, model_factory):
