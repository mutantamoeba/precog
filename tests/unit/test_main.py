...
class TestMainEntryPoint:
    ...
    def test_version_accessible(self, cli_runner):
    ...
    def test_command_groups_registered(self, cli_runner):
...
class TestSubcommandAccess:
    ...
    def test_kalshi_help(self, cli_runner):
        """Test kalshi subcommand help is accessible."""
        result = cli_runner.invoke(app, ["kalshi", "--help"])

        assert result.exit_code == 0
        output_lower = strip_ansi(result.stdout).lower()
        assert "balance" in output_lower or "markets" in output_lower

    def test_espn_help(self, cli_runner):
        """Test espn subcommand help is accessible."""
        result = cli_runner.invoke(app, ["espn", "--help"])
    ...
    def test_system_help(self, cli_runner):
...
class TestInvalidCommands:
    ...
    def test_old_command_format_fails(self, cli_runner):
    ...
    def test_old_config_show_fails(self, cli_runner):
        """Test that old config-show command is rejected."""
        result = cli_runner.invoke(app, ["config-show"])

        assert result.exit_code != 0

    def test_old_health_check_fails(self, cli_runner):
